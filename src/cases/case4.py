"""Case 4 — NGCC on-site, off-grid baseline (v2.5 §B).
...
cooling. No grid imports. This is the deployment-realistic fossil
alternative to the nuclear cases.

Like Case 0, this runs as an independent computation rather than as part
of the unified Cases 1-3 MILP (v2.5 §2.1):
...
def solve_case4(
    ...
    """Compute Case 4 NGCC on-site dispatch and TAC.

    Args:
        cfg: ``RunConfig`` loaded for ``case_id=4``.
        ts: Aligned hourly time series (one ERCOT year).
        pue: Override the PUE setpoint (defaults to ``cfg.base.physics.pue_default``).

    Returns:
        ``Case4Result`` with per-hour dispatch + annualized accounting.

    Raises:
        ValueError: missing required config blocks, or any-hour demand
            exceeds installed NGCC or VCC capacity.
    """
    if cfg.case.case_id != 4:
        raise ValueError(f"solve_case4 requires case_id=4, got {cfg.case.case_id}")
    if cfg.case.ngcc is None or cfg.case.vcc is None:
        raise ValueError("Case 4 requires both ngcc and vcc blocks in plant_case4.yaml")
    ...
    if Q_capacity is None or NGCC_capacity is None:
        raise ValueError(
            "Case 4 requires capacities.electric_chiller_capacity_MWth and "
            "capacities.ngcc_capacity_MWe in plant_case4.yaml"
        )
