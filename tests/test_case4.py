"""Tests for src.cases.case4 — NGCC on-site off-grid baseline."""
...
def cfg():
    return load_config(case_id=4, project_root=PROJECT_ROOT)
...
def test_case4_loads_with_ngcc_enabled(cfg):
    assert cfg.case.case_id == 4
...
def test_case4_smoke_returns_result(cfg, ts_2023_168h):
    result = solve_case4(cfg, ts_2023_168h)
    assert result.tac_usd_per_yr > 0


def test_case4_off_grid_balance(cfg, ts_2023_168h):
...
def test_case4_fuel_consumption_matches_efficiency(cfg, ts_2023_168h):
...
def test_case4_fuel_cost_uses_year_resolved_henry_hub(cfg, ts_2023_168h):
...
def test_case4_2022_fuel_more_expensive_than_2024(cfg):
    """v2.5 S2: 2022 HH spike → Case 4 fuel cost ≈ 3× 2024."""
    ...
    r22 = solve_case4(cfg, ts22)
    r24 = solve_case4(cfg, ts24)
    # 2022 mean HH $6.45 vs 2024 $2.19 → ratio ~ 2.9x
    ratio = r22.fuel_annual_usd / r24.fuel_annual_usd
    assert 2.0 < ratio < 4.0


def test_case4_direct_emissions_match_emission_factor(cfg, ts_2023_168h):
...
def test_case4_lifecycle_emissions_include_upstream_ch4(cfg, ts_2023_168h):
...
def test_case4_tac_components_sum_correctly(cfg, ts_2023_168h):
    result = solve_case4(cfg, ts_2023_168h)
    parts = (
...
def test_case4_higher_pue_increases_fuel_and_tac(cfg, ts_2023_168h):
    r110 = solve_case4(cfg, ts_2023_168h, pue=1.10)
    r150 = solve_case4(cfg, ts_2023_168h, pue=1.50)
